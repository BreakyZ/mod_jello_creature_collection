...
::Const.PerchtPerks.Perks <- [
	[
		...
		{
			ID = "perk.krampus_charge_alt",
			Script = "scripts/skills/perks/perk_percht_horn_charge",
			Name = "Horn Charge",
			Tooltip = "Charge towards a position within 2 tiles, ramming a target at the destination with your horns to stun them.",
			Icon = "ui/perks/jcc_perk_01.png",
			IconDisabled = "ui/perks/jcc_perk_01_sw.png"
		}
	],
	[
		{
...
::Const.PerchtPerks.LookupMap <- {};

for( local row = 0; row < this.Const.PerchtPerks.Perks.len(); row = ++row )
{
	for( local i = 0; i < this.Const.PerchtPerks.Perks[row].len(); i = ++i )
	{
		local perk = this.Const.PerchtPerks.Perks[row][i];
		perk.Row <- row;
		perk.Unlocks <- row;
		gt.Const.PerchtPerks.LookupMap[perk.ID] <- perk;
	}
}

::Const.PerchtPerks.findById <- function ( _perkID )
{
	if (_perkID != null && _perkID in this.Const.PerchtPerks.LookupMap)
	{
		return this.Const.PerchtPerks.LookupMap[_perkID];
	}
