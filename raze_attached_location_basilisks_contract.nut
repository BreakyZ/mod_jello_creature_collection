this.raze_attached_location_basilisks_contract <- this.inherit("scripts/contracts/contract", { //Const.World.Spawn.BasiliskMED
	m = {
		Destination = null,
		Settlement = null
	},
	...
	//Premise
// 1) a noble is annoyed that a swarm of basilisks butchered one of his patrols
// 2) the noble directs you to a separate faction, where the basilsiks are currently residing
// 3) the noble says he needs it done now, and he will 'amend any bad blood' between you and the other faction after the job is done
// 4) go to x faction, attack their attached location and kill the basilisks
// 5) if any basilisks escape, it affects your reward
	...
	function create()
	{
		...
		this.m.Flags.set("BasilisksEscaped", 0);
	...
	function createStates()
	{
		...
		this.m.States.push({
			...
			function onActorRetreated( _actor, _combatID )
			{
				...
				{
					this.Flags.set("BasilisksEscaped", this.Flags.get("BasilisksEscaped") + 1);
				}
			}
			...
			function onRetreatedFromCombat( _combatID )
			{
				if (_combatID == "RazeLocation")
				{
					this.Flags.set("BasilisksEscaped", 100);
				}
				else if (_combatID == "Defend")
				{
					this.Flags.set("IsDone", true);
		...
		this.m.States.push({
			...
			function update()
			{
				...
				{
					if (this.Flags.get("BasilisksEscaped") == 0)
					{
						this.Contract.setScreen("Success1");
					}
					else if (this.Math.rand(1, 100) >= this.Flags.get("BasilisksEscaped") * 10)
					{
