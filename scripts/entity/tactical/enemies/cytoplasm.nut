this.cytoplasm <- this.inherit("scripts/entity/tactical/actor", {
	m = 
	{
		Size = 1,
		Variant = 1,
	...
	function create()
	{
		this.m.Type = this.Const.EntityType.Cytoplasm;
		//this.m.BloodType = this.Const.BloodType.Red;
		this.m.XP = this.Const.Tactical.Actor.Cytoplasm.XP;
		...
		if (_tile != null && this.getSize() > 1)
		{
			...
			if (freeTiles.len() != 0)
			{
				...
				while (n >= 0 && freeTiles.len() >= 1)
				{
					...
					freeTiles.remove(r);
					local rock = this.Tactical.spawnEntity("scripts/entity/tactical/enemies/cytoplasm", tile.Coords.X, tile.Coords.Y);
					...
					if (this.getWorldTroop() != null && ("Party" in this.getWorldTroop()) && this.getWorldTroop().Party != null && !this.m.WorldTroop.Party.isNull())
					{
						...
						if (this.getSize() == 3)
						{
							e = this.Const.World.Common.addTroop(this.getWorldTroop().Party.get(), {
								Type = this.Const.World.Spawn.Troops.CytoplasmMEDIUM
							}, false);
						}
						else
						{
	...
	function onInit()
	{
		...
		baseProperties.setValues(this.Const.Tactical.Actor.Cytoplasm);
	...
	function grow( _instant = false )
	{
		...
		if (this.m.Size == 2)
		{
			...
			{
				this.m.WorldTroop.Script = this.Const.World.Spawn.Troops.CytoplasmMEDIUM.Script;
				this.m.WorldTroop.Strength = this.Const.World.Spawn.Troops.CytoplasmMEDIUM.Strength;
			}
		...
		else if (this.m.Size == 3)
		{
			...
			if (this.m.WorldTroop != null && ("Party" in this.m.WorldTroop) && this.m.WorldTroop.Party != null && !this.m.WorldTroop.Party.isNull())
			{
				this.m.WorldTroop.Script = this.Const.World.Spawn.Troops.CytoplasmHIGH.Script;
				this.m.WorldTroop.Strength = this.Const.World.Spawn.Troops.CytoplasmHIGH.Strength;
			}
		}
	...
	function shrink( _instant = false )
	{
		...
		else if (this.m.Size == 1)
		{
			...
			if (this.m.WorldTroop != null && ("Party" in this.m.WorldTroop) && this.m.WorldTroop.Party != null)
			{
				this.m.WorldTroop.Script = this.Const.World.Spawn.Troops.Cytoplasm.Script;
				this.m.WorldTroop.Strength = this.Const.World.Spawn.Troops.Cytoplasm.Strength;
			}
