this.jcc_dragon_hunt_contract <- this.inherit("scripts/contracts/contract", {
	m = {
		Target = null,
		Dude = null,
	...
	function create()
	{
		...
		this.m.Type = "contract.dragon_slaying";
		this.m.Name = "Dragon Slaying";
	...
	function start()
	{
		...
		this.m.Flags.set("DamselName", this.generateName());
	...
	function createStates()
	{
		this.m.States.push({
			...
			function start()
			{
				this.Contract.m.BulletpointsObjectives = [
					"Hunt down the dragons around " + this.Contract.m.Home.getName()
				];
			...
			function end()
			{
				...
				{
					this.Flags.set("IsDamselInDistress", true);
				}
				...
				local party = this.World.FactionManager.getFactionOfType(this.Const.FactionType.Beasts).spawnEntity(tile, "Drache", false, this.Const.World.Spawn.JccDragon, 100 * this.Contract.getDifficultyMult() * this.Contract.getScaledDifficultyMult(), this.Contract.getMinibossModifier());
		...
		this.m.States.push({
			...
			function onTargetAttacked( _dest, _isPlayerAttacking )
			{
				...
				else if (this.Flags.get("IsDamselInDistress"))
				{
					this.Contract.setScreen("DamselDistress");
					this.World.Contracts.showActiveContract();
				}
				else if (this.Flags.get("IsLindwurmFight"))
				{
					this.Contract.setScreen("LindwurmFight");
					this.World.Contracts.showActiveContract();
				}
			...
			{
				_entity.getFlags().set("IsDamselInDistress", true);
				_entity.setName(this.Flags.get("DamselName"));
			}

			function onActorKilled( _actor, _killer, _combatID )
			{
				if (_combatID != "JccDragons")
				{
					return;
				}

				if (_actor.getType() == this.Const.EntityType.JccNoble)
				{
					this.Flags.increment("DamselDead");
				}
			}
			...
			Options = [
				{
					...
					function getResult()
					{
						...
						this.Const.World.Common.addUnitsToCombat(p.Entities, this.Const.World.Spawn.Lindwurm, 100 * this.Contract.getDifficultyMult() * this.Contract.getScaledDifficultyMult(), this.Const.Faction.Enemy);
			...
			Options = [
				{
					...
					function getResult()
					{
						...
						p.PlayerDeploymentType = this.Const.Tactical.DeploymentType.Edge;
						p.EnemyDeploymentType = this.Const.Tactical.DeploymentType.Random;
						this.Const.World.Common.addUnitsToCombat(p.Entities, this.Const.World.Spawn.Direwolves, 150 * this.Contract.getDifficultyMult() * this.Contract.getScaledDifficultyMult(), this.Const.Faction.Enemy);
						this.World.Contracts.startScriptedCombat(p, false, true, true);
						return 0;
					}
			...
			Options = [
				{
					...
					function getResult()
					{
						...
						this.World.Contracts.startScriptedCombat(p, false, true, true);
						this.Const.World.Common.addUnitsToCombat(p.Entities, this.Const.World.Spawn.Noble, 90, this.Contract.getFaction());
						return 0;
					}
