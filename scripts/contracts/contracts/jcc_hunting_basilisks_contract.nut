this.jcc_hunting_basilisks_contract <- this.inherit("scripts/contracts/contract", {
	m = {
		Beastslayer = null,
		Target = null,
		Dude = null,
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Task",
			Title = "Negotiations",
			Text = "[img]gfx/ui/events/event_43.png[/img]{%employer% sinks into his chair and stares through you. Peasants with pitchforks swan around outside, voices are raised beyond the window and then start to dissipate moments later. %employer% runs his hands through his hair and meets your gaze. %SPEECH_ON%I have been in talks with the leaders of that mob outside for the best part of the day. Their crops are trampled, stolen or half eaten in their fields. Some even say their livestock has gone missing.%SPEECH_OFF% He rises from the chair and moves towards the window. %SPEECH_ON%They want me to replace everything they lost, Sellsword. But the best I can do is put an end to it. Some of these madmen claim chickens larger than a man swallowed their lambs whole. I don\'t care who or what has been doing this, but make sure you put an end to it.%SPEECH_OFF% %employer% turns to face you. %SPEECH_ON%Do I have your interest, Sellsword?%SPEECH_OFF% | A group of men leave a much finer building than any of them could possibly claim ownership to combined, they are dressed in clothes clearly not worn often, the fine cloth contrasting horribly against their sun-blotched skin and calloused hands. They move past %companyname% spitting curses and fighting amongst themselves. You enter the building and there %employer% sits, sipping wine and leafing through parchment papers. He speaks to you without acknowledging your presence. %SPEECH_ON%I don\'t doubt you saw that miserable bunch that just left. Those simple fools claim that a giant chicken swallowed their dog whole and trampled their crops. Tosh! To me, Sellsword, it sounds like your standard corpse-eater or perhaps some kind of elaborate brigand scheme to frighten us.%SPEECH_OFF% He moves some papers into a new pile and produces a bag of crowns from a drawer, seemingly with rehearsed effort. His tone becomes more moody and disapproving. %SPEECH_ON%I keep a rainy day fund for these things, whatever the problem is - fix it and it\'s yours.%SPEECH_OFF%}", // 1-3 more needed
			Image = "",
			List = [],
			ShowEmployer = true,
			ShowDifficulty = true,
		...
		this.m.Screens.push({ //flavour
			ID = "Banter",
			Title = "Along the way...",
			Text = "[img]gfx/ui/events/event_25.png[/img]{You smell it before you see it, but you still almost trip over the half decomposed carcass of a giant bird. %randombrother% is not so lucky, and trips over the wing to disturb a flurry of flies and worms. %SPEECH_ON%Farkin\' \'ell%SPEECH_OFF% As he brushes the slime from his boots, you notice the sheer size of this monster and the pointed beak which is still sharp enough cut your hand on. The eyes have been picked out and the feathers are ruined - what meat remains has spoiled here in the heat on the forest floor. | The %companyname% trapises further into the woods, part of the land far from the eyes and care of men. A large nest, much larger than that of what you are hunting, lies in a ditch below a thick treeline. Bones of large animals and men litter the nest and surrounding area - some remains are crushed or torn apart while others are only missing hands or feet. %randombrother% picks through the remains %SPEECH_ON%Lizard nest. Not what we\'re here for, long gone by the looks of things...%SPEECH_OFF% You noticed some eggs within the nest differ slightly in size and shape. | You come across a ditch that is filled with a stagnant pool of filth and remains. Various corpses of both animal and man litter this place, some obviously dragged dead while others were more or less alive.}",
			Image = "",
			List = [],
			Options = [
				{
...
//		});
		...
		this.m.Screens.push({ //post battle item loot - get extra feathers
			...
			function start()
			{
				local item = this.new("scripts/items/misc/jcc_basilisk_feathers_item");
		...
		this.m.Screens.push({ //post battle bro hire
			ID = "Survivor",
			Title = "After the battle...",
			Text = "[img]gfx/ui/events/event_128.png[/img]{After the noise of battle dies down and the smell rises, you notice a discarded and broken bow stamped into the mud at the edge of the nest clearing. A smaller basilisk has met it\'s end courtesy of several arrows firmly placed in it\'s skull. A tree branch cracks and above you, a hunter in tattered clothes and various wounds smiles down upon you as if he was a mocking, filthy angel. %SPEECH_ON%Much faster than they look, eh?%SPEECH_OFF% The tree he has claimed is pocked with deep gouges and claw marks from every angle, only abruptly ending a few feet up the trunk. The anger of each strike is so fierce that it almost seems as if the objective was to bring the tree itself down. %SPEECH_ON%Got the small one but that only made the bigger ones more angry. Meat is good if you know where to cut and there\'s plenty of it... say, you got room for one more?%SPEECH_OFF%}",
			Image = "",
			List = [],
			Characters = [],
			Options = [
	...
	function onUpdateScore()
	{
		...
		{
			if (bro.getBackground().getID() == "background.beast_slayer" || bro.getBackground().getID() == "background.jcc_guildmaster")
			{
