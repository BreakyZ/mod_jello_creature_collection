this.ai_merge_cyto <- this.inherit("scripts/ai/tactical/behavior", {
	m = {
		PossibleSkills = [
			"actives.merge_cyto"
		],
		...
		if (this.m.Skill == null)
		{
			...
			else if (!engagedInMelee && _entity.isAbleToWait() && _entity.getActionPoints() <= 2)
			{
				...
				for( local j = 0; j < 6; j = ++j )
				{
					...
					else if (this.myTile.getNextTile(j).IsOccupiedByActor && this.myTile.getNextTile(j).getEntity().getType() == this.Const.EntityType.Cytoplasm && this.myTile.getNextTile(j).getEntity().getSize() == _entity.getSize())
					{
