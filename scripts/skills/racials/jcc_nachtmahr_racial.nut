this.jcc_nachtmahr_racial <- this.inherit("scripts/skills/skill", {
	m = {
		TimesWaited = 0
	},
	function create()
	{
		this.m.ID = "racial.jcc_nachtmahr";
		this.m.Name = "";
		this.m.Description = "";
	...
	function teleport( _tag )
	{
		...
		foreach( a in allies )
		{
			if (a.getHitpoints() > 0 && a.getType() == this.Const.EntityType.JccNachtmahr)
			{
