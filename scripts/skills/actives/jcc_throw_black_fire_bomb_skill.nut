this.jcc_throw_black_fire_bomb_skill <- this.inherit("scripts/skills/skill", {
	m = {},
	function create()
	{
		this.m.ID = "actives.jcc_throw_black_fire_bomb";
		this.m.Name = "Throw Hellfire Pot";
		this.m.Description = "Ignite and throw a pot filled with highly flammable liquids towards a target, where it will shatter and set the area ablaze. Anyone ending their turn inside the burning area will catch fire and take damage - friend and foe alike. Unaffected by snow.";
		this.m.Icon = "skills/jcc_throw_firebomb.png";
		this.m.IconDisabled = "skills/jcc_throw_firebomb_sw.png";
		this.m.Overlay = "jcc_throw_firebomb";
		this.m.SoundOnUse = [
		...
		this.m.ProjectileType = this.Const.ProjectileType.JccFirebomb;
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "Set an area of [color=" + this.Const.UI.Color.DamageValue + "]7[/color] tiles ablaze with fire for 3 rounds. Water and snow can not burn."
		});
		ret.push({
			id = 6,
			type = "text",
	...
	function onApply( _data )
	{
		...
		{
			if (tile.Type != this.Const.Tactical.TerrainType.ShallowWater && tile.Type != this.Const.Tactical.TerrainType.DeepWater)
			{
